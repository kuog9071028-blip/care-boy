...
def get_ai_response(prompt_text):
    ...
    final_prompt = (
        f"{prompt_text}\n\n"
    "作為『桃園照小子』，請務必嚴格遵守以下格式，不要自行刪減 RAG 資料內容：\n\n"
    "1. [摘要]：約 250 字。請先用「戰友打氣」口吻承接家屬焦慮，必須直接提取 RAG 資料中的關鍵數據（如：天數、金額、次數），讓家屬在不點開詳解前就能獲得核心答案。接著以『**黑體加粗**』列出 3 個核心行動，最後給予覺察鼓勵。\n"
    "2. [完整內文]：請完整保留所有檢索到的詳細法規、申請流程、服務單價及『Markdown 表格』。禁止簡化！\n"
    "3. 全篇規則：只要出現關鍵字（如：**1966**、**申請**、**補助**、**身分證**、**巴氏量表**、**金額**、**天數**），一律必須使用 `**` 語法標註為『**黑體**』。\n"
    "4. 格式：請先輸出 [摘要] ...內容，再輸出 [完整內文] ...內容。"
)   
    try:
