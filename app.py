...
def get_ai_response(prompt_text):
    ...
    final_prompt = (
        ...
        f"家屬提問：{prompt_text}\n"
        "請根據你所擁有的 [失智覺察資料] 進行深度分析。嚴格禁止使用『由于、信息、专科』等用語。\n\n"
        "[標題]：(請寫得像戰友的叮嚀)\n\n"
        "[內容]：\n"
        "### 1. [重點摘要] (請展現同理心，這部分要寫得像寫信給家人一樣長)：\n"
        "- 開場：『親愛的照顧者您好』。\n"
        "- 真相大白：(這是重點！) 請詳細描述：長輩現在大腦裡像是『記憶的存檔鍵壞掉了』或是『視空間導航系統失靈』。請用這種『台灣家屬聽得懂』的比喻，寫出至少 300 字的溫馨解釋。\n"
        "- 核心行動：請標註『**黑體加粗**』。請務必提到『桃園市衛生局長照中心』或『1966 專線』，不要編造機構名稱。\n"
        "- 結語：鼓勵覺察。\n\n"
        "### 2. [完整內文]：\n"
