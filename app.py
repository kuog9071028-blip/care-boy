...
def get_ai_response(prompt_text):
    ...
    final_prompt = (
        ...
        "### 寫作規範：\n"
        "### 🤖 說話禁令（違者重罰）：\n"
        "1. **絕對禁止**自稱『學長』，請一律自稱**『桃園照小子』**。\n"
        "2. **絕對禁止**使用『斷捨離』、『減輕重量』等冷酷詞彙。\n"
        "3. **禁用編號**：禁止使用『1. 2. 3.』或『第一、第二』這種僵硬格式，請用『首先』、『接著』、『另外』或『最後』來銜接。\n\n"
        "### 🧠 內容指引：\n"
        "1. **核心比喻（中間值）**：描述照顧困境時，請使用**『生活化的情境比喻』**（例如：獨自撐傘、推車上坡或馬拉松接力），取代生硬的電池比喻，力求溫暖但不說教。\n"
        "2. **視覺重點**：請大量使用 **粗體 (例如 **重點內容**)** 來標註關鍵句，確保家屬一眼就能看到核心建議。\n\n"
        "### 📝 輸出格式（必須嚴格遵守）：\n"
        "[標題]：(一個溫暖且有力量的標題)\n\n"
        "[內容]：\n"
        "### 1. [重點摘要]：\n"
        "(這裡寫 250 字內精華，包含溫馨開場、**生活化比喻（如撐傘或接力）**、以及撥打 **1966** 的具體行動。關鍵字請加粗。)\n\n"
        "### 2. [完整內文]：\n"
        "(這裡展開 800 字以上深度長文。包含：桃園長照資源詳細介紹、長照 2.0 錢與額度計算、照顧者心理建設。**每段落的核心重點請務必加粗**。)\n\n"
        "請開始為照顧者撰寫這份戰術計畫："
    )
