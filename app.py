...
def get_ai_response(prompt_text):
    ...
    final_prompt = (
        ...
        "### 寫作規範：\n"
        "1. **格式嚴格執行**：必須包含 [標題]、[內容]、[重點摘要]、[完整內文] 等標籤。\n"
        "2. **文字量要求**：這是一封暖心的長信，總字數請挑戰 800 字以上。\n\n"
        "[標題]：(請寫出一個戰友般的叮嚀)\n\n"
        "[內容]：\n"
        "### 1. [重點摘要]：\n"
        "- **開場同理**：親愛的照顧者您好，這段路真的辛苦了... (請發揮同理心寫至少 150 字)。\n"
        "- **病理真相**：(重點！) 請詳細用溫馨比喻（如記憶存檔鍵壞了）解釋長輩的症狀，這部分請寫得極其詳盡，讓家屬釋懷。\n"
        "- **核心行動**：請標註『**黑體加粗**』，提到桃園 1966 與在地資源。\n\n"
        "### 2. [完整內文]：\n"
        "- **桃園在地戰略**：根據家屬困擾，深入介紹 2-3 個桃園具體資源（如失智據點、愛心手鍊等）。\n"
        "- **錢要花在刀口上**：詳細解釋台灣長照 2.0 的 CMS 等級、額度與自負額（一般戶 16%）的概念，鼓勵家屬利用補助換取喘息。\n"
        "- **戰友的結語**：鼓勵家屬覺察自己的情緒，給予最後的溫暖喊話。\n\n"
        "請開始撰寫這份充滿溫度的長照戰略計畫："
    )
    try:
        # 4. 呼叫 Gemini 產生內容
